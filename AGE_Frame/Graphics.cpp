...
void AGE_Frame::OnGraphicSelect(wxCommandEvent &event)
{
    ...
    if(selections > 0)
    {
        ...
        SetStatusText("Selections: "+std::to_string(GraphicIDs.size())+"    Selected sprite: "+std::to_string(GraphicIDs.front()), 0);
...
void AGE_Frame::CreateGraphicsControls()
{
    ...
    Deltas_Import->Bind(wxEVT_BUTTON, [this](wxCommandEvent &event)
    {
        ...
        {
            wxBusyCursor WaitCursor;
            ifstream infile(std::string(dd.GetPath()));
            string line;
            while(getline(infile, line))
            {
                ...
                for(size_t loop = 1, spriteID = static_cast<size_t>(-1), deltaID = static_cast<size_t>(-1); loop < pieces.GetCount(); ++loop)
                {
                    ...
                    case 1: // ID
                    {
                        try
                        {
                            spriteID = std::stoul(pieces[1].ToStdString());
//                            spriteID = lexical_cast<uint32_t>(pieces[1]);
                    ...
                    case 2: // Either index of the delta or the sprite number of the delta.
                    {
                        try
                        {
                            deltaID = std::stoul(pieces[2].ToStdString());
                        }
                        catch(const invalid_argument&)
                        {
                    ...
                    case 4: // Offset X
                    {
                        ...
                        try
                        {
                            dataset->Graphics[spriteID].Deltas[deltaID].OffsetX = std::stoul(pieces[4].ToStdString());
                        }
                        catch(const invalid_argument&)
                        {
                    ...
                    case 5: // Offset Y
                    {
                        ...
                        {
                            dataset->Graphics[spriteID].Deltas[deltaID].OffsetY = std::stoul(pieces[5].ToStdString());
                        }
                        catch(const invalid_argument&)
                        {
