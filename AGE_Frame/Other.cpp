...
void AGE_Frame::OnMenuOption(wxCommandEvent &event)
{
    switch(event.GetId())
    {
        ...
        case eCompileList:
        {
            ...
            {
                slp_csv.AddLine(std::to_string(dataset->Graphics[sprite].SLP)
                            +','+dataset->Graphics[sprite].FileName
                            +','+dataset->Graphics[sprite].Name
                            +','+std::to_string(dataset->Graphics[sprite].FrameCount)
                            +','+std::to_string(dataset->Graphics[sprite].AngleCount));
            }
                ...
                if(sprite < dataset->Graphics.size() && dataset->GraphicPointers[sprite])
                {
                    if(!dataset->Graphics[sprite].FileName.empty())
                    {
                        obj_csv.AddLine(dataset->Civs[civ].Name
                            +','+std::to_string((int)dataset->Civs[civ].IconSet)
                            +','+std::to_string(unit)
                            ...
                            +','+dataset->Graphics[sprite].Name
                            +','+dataset->Graphics[sprite].FileName
                            +','+std::to_string((int)dataset->Graphics[sprite].Layer)
                            +','+std::to_string(dataset->Graphics[sprite].FrameCount)
                            +','+std::to_string(dataset->Graphics[sprite].AngleCount));
                    }
                    if(depth == 1)
...
bool AGE_Frame::LoadSLP(AGE_SLP *graphic)
{
    ...
    if(UseLooseSLP)
    {
        if(!graphic->filename.empty())
        {
            ...
            string plainName = PathSLP.ToStdString() + "/" + graphic->filename.ToStdString();
    ...
    if(LooseHD)
    {
        ...
        if(!graphic->filename.empty())
        {
            ...
            for(int i=0; i < folders.size(); ++i)
            {
                graphic->slp = GG::LoadSLP(folders[i].ToStdString() + graphic->filename.ToStdString() + ".slp");
                if(graphic->slp) return true;
            }
            folders.clear();
        }
